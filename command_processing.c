...
/**
 * determine_chain - evaluate if the current character
...
int determine_chain(info_t *info, char *buf, size_t *p)
{
size_t j = *p;
...
/**
 * verify_chain - check whether we should continue chaining
 * @info: the parameter struct
 * @buf: the character buffer
 * @p: address of the current position in buf
...
/**
 * substitute_alias - replace aliases in the tokenized string
 * @info: the parameter struct
 * Goodnews and Olatunji
...
/**
 * substitute_vars - replace variables in the tokenized string
 * @info: the parameter struct
 * Goodnews and Olatunji
 * Return: 1 if replaced, 0 otherwise
 */
...
/**
 * substitute_string - replace a string
